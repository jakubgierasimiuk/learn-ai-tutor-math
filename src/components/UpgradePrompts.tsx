...
import { Zap, Crown, Sparkles, Users, X } from 'lucide-react';
...
export const UpgradePrompts = ({ context = 'chat', compact = false }: UpgradePromptsProps) => {
  ...
  // Contextual upgrade prompts based on token usage
  const renderContextualPrompt = () => {
    if (!subscription || subscription.subscription_type !== 'free') return null;

    const remaining = getRemainingTokens();
    const percentage = getUsagePercentage();

    if (context === 'chat' && percentage >= 75) {
      return (
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-orange-800">Zostało Ci tylko {remaining} tokenów</p>
                  <p className="text-sm text-orange-600">Ulepsz plan, aby kontynuować naukę bez ograniczeń</p>
                </div>
              </div>
