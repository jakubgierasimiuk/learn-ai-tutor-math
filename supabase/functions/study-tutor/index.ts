...
async function handleSkillRecognition(req: Request): Promise<Response> {
  try {
    ...
    console.log('Skill recognition request for user:', user.user?.id)
    ...
    // Create skill recognition prompt
    const skillsList = skills.map(skill => 
      `- ${skill.name} (${skill.department}): ${skill.description} [ID: ${skill.id}]`
    ).join('\n')

    const recognitionPrompt = `Analizuj następującą wiadomość użytkownika i wybierz najlepiej pasującą umiejętność matematyczną z dostępnej listy.
...
{
  "skillId": "uuid-wybranej-umiejętności",
  "skillName": "nazwa umiejętności",
  "confidence": liczba_od_0_do_1,
  "reasoning": "krótkie uzasadnienie wyboru"
}

Jeśli żadna umiejętność nie pasuje dobrze (confidence < 0.6), zwróć null jako skillId.`

    // Call OpenAI for skill recognition
    const openaiResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { 
            role: 'system', 
            content: 'Jesteś ekspertem w rozpoznawaniu umiejętności matematycznych na podstawie pytań uczniów. Zawsze odpowiadaj w poprawnym formacie JSON.' 
          },
    ...
    if (!openaiResponse.ok) {
      ...
      return new Response(JSON.stringify({ error: 'Failed to recognize skill' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    try {
      const recognitionResult = JSON.parse(aiResponse)
      ...
      return new Response(JSON.stringify({ error: 'Failed to parse skill recognition result' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
...
serve(async (req) => {
  ...
  } else if (pathname === '/recognize-skill') {
    return handleSkillRecognition(req)
  } else {
