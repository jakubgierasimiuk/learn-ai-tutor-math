...
SELECT 
    ...
    'theory', COALESCE(
      (
        SELECT jsonb_build_object(
          'theory_text', stc.theory_text,
          'key_formulas', stc.key_formulas,
          'time_estimate', stc.time_estimate,
          'difficulty_level', stc.difficulty_level,
      ...
      (
        SELECT jsonb_agg(
          jsonb_build_object(
            ...
            'time_estimate', se.time_estimate
          )
        )
  ...
  jsonb_build_object(
    ...
    'chapter_tag', s.chapter_tag
  ) as metadata,
  (
    -- Check if skill has complete content (theory + examples + exercises with substantial data)
    EXISTS(SELECT 1 FROM skill_theory_content stc WHERE stc.skill_id = s.id AND stc.is_active = true AND LENGTH(stc.theory_text) > 100) AND
