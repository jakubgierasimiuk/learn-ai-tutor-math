-- Update existing topics table with new columns
ALTER TABLE public.topics ADD COLUMN IF NOT EXISTS category TEXT NOT NULL DEFAULT 'mathematics';
ALTER TABLE public.topics ADD COLUMN IF NOT EXISTS prerequisites TEXT[];
ALTER TABLE public.topics ADD COLUMN IF NOT EXISTS learning_objectives TEXT[] NOT NULL DEFAULT '{}';
ALTER TABLE public.topics ADD COLUMN IF NOT EXISTS estimated_time_minutes INTEGER DEFAULT 30;
...
-- Update existing topics with new data
UPDATE public.topics SET 
  description = 'Podstawowe operacje matematyczne: dodawanie, odejmowanie, mnożenie, dzielenie',
  learning_objectives = ARRAY['Opanowanie podstawowych operacji arytmetycznych', 'Rozumienie kolejności wykonywania działań', 'Umiejętność wykonywania obliczeń w pamięci'],
  estimated_time_minutes = 45
WHERE name = 'Algorytmy i struktury danych';

-- Insert additional math topics
