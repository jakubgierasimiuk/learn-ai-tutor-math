...
ADD COLUMN class_level_text TEXT;

-- Skopiowanie istniejących wartości class_level do class_level_text
UPDATE skills SET class_level_text = class_level::text;
...
WHERE department IN ('algebra', 'functions', 'geometry', 'calculus', 'analiza_matematyczna', 'sequences')
   AND class_level IN (1, 2, 3, 4);

-- Poprawne mapowanie klas licealnych zgodnie z polską podstawą programową
UPDATE skills SET class_level_text = '1 LO' WHERE class_level = 1 AND school_type = 'liceum';
UPDATE skills SET class_level_text = '2 LO' WHERE class_level = 2 AND school_type = 'liceum';
UPDATE skills SET class_level_text = '3 LO' WHERE class_level = 3 AND school_type = 'liceum';
UPDATE skills SET class_level_text = '4 LO' WHERE class_level = 4 AND school_type = 'liceum';
...
UPDATE skills SET class_level_text = '1 LO', school_type = 'liceum', is_hidden = false
WHERE name ILIKE '%liczby rzeczywiste%' 
   OR name ILIKE '%działania na liczbach rzeczywistych%'
...
UPDATE skills SET class_level_text = '2 LO', school_type = 'liceum', is_hidden = false
WHERE name ILIKE '%kwadratowe%'
   OR name ILIKE '%wartość bezwzględna%'
   OR name ILIKE '%ciągi%'
   OR name ILIKE '%parabola%'
   OR name ILIKE '%wielomianowe%';

-- 3 LO: Trygonometria, funkcje wykładnicze/logarytmiczne (poziom rozszerzony)
UPDATE skills SET class_level_text = '3 LO', school_type = 'liceum', is_hidden = false
...
UPDATE skills SET class_level_text = '4 LO', school_type = 'liceum', is_hidden = false
...
CREATE INDEX idx_skills_class_level_text ON skills(class_level_text);